...
---
## 数据生成与原理 (Data Generation & Principles)
...
If you want to reproduce the data or conduct new explorations, use these tools:
-   **`tools/wand_eval_tree/exhaust_wands.py`**: 魔杖全量穷举脚本。通过多进程调用 LuaJIT 模拟 Noita 法术系统。
    *(Wand exhaustion script. Uses multi-processing and LuaJIT to simulate the Noita magic system.)*
-   **`tools/wand_eval_tree/shard_csv.py`**: 数据分片脚本。将生成的巨量 CSV 数据切分为前端可加载的文本文件。
    *(Data sharding script. Splits the massive CSV output into small chunks for the web frontend.)*
-   **`tools/wand_eval_tree/main.lua`**: 支持 LuaJIT 的 Noita 魔杖环境模拟内核。
    *(The Noita wand simulation core, optimized for LuaJIT.)*

### 2. 原理解析 (Understanding the Logic)
...
-   📄 **[AD_SOURCE_NOTES.md](tools/wand_eval_tree/AD_SOURCE_NOTES.md)**: 包含了对模拟引擎核心设计逻辑的解析。
    *(Contains analysis and design documentation for the simulation engine core.)*

### 3. 如何使用 (How to Use)
1.  确保本地有 `Python 3.x` 环境和 `LuaJIT`。
    *(Ensure you have `Python 3.x` and `LuaJIT` installed.)*
2.  修改 `tools/wand_eval_tree/user_config.lua`，将 `noita_path` 和 `data_path` 填入你本地已提取的 Noita `data/` 目录路径。
    *(Modify `tools/wand_eval_tree/user_config.lua` to set `noita_path` and `data_path` to your extracted Noita `data/` directory.)*
3.  在 `exhaust_wands.py` 中根据本地环境调整 `LUAJIT_PATH`。
    *(Verify `LUAJIT_PATH` in `exhaust_wands.py` points to your LuaJIT executable.)*
4.  运行 `python tools/wand_eval_tree/exhaust_wands.py` 开始生成生成组合。
    *(Run `python tools/wand_eval_tree/exhaust_wands.py` to start the generation.)*
5.  运行 `python tools/wand_eval_tree/shard_csv.py` 进行数据分片（输出到 `data/` 目录）。
    *(Run `python tools/wand_eval_tree/shard_csv.py` for data sharding, results move to `data/`.)*
